...
<?php 
for($n=1; $n <= $itemPackedDetail->total; $n++){
	?>
    ...
    border-radius: 10px;">
        ...
        <p><strong>Store Name </strong> {{$customerdetail->store_name}}</p>
         <p><strong>Customer Address </strong> {{$customerdetail->business_street_address}}</p>
             <p><strong>Order No. </strong><span style="font-weight: 600; font-size: 25px">{{@$itemPackedDetail->order_number}}</span></p>
             <p><strong>Order Date </strong>{{date('d/m/Y', strtotime(@$itemPackedDetail->created_at))}}</p>

             <p><strong>Nug </strong><span style="font-weight: 600; font-size: 25px"><?php echo $n.'/'.$itemPackedDetail->total;?></span></p>
          </div>
