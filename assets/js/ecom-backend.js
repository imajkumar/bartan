...
$(document).ready(function() {
    ...
    function checkDocCertificate() {
        docCertificateValidate = 0;
        if ($("#business_gst_number").val() != "" && ($("#gst_certificate").val() != "" || $("#gst_certificate_old").val() != "")) {

            docCertificateValidate += 1;
            //return $('#business_gst_number').after('<div id="business_gst_number_errorMsg" class="error">Please Select Any two document from first four options are mandatory</div>');
        }
        if ($("#adhar_number").val() != "" && ($("#adhar_card").val() != "" || $("#adhar_card_old").val() != "")) {
            docCertificateValidate += 1;
        }
        if ($("#driving_license").val() != "" && ($("#driving_license").val() != "" || $("#driving_license_old").val() != "")) {
            docCertificateValidate += 1;
        }
        // if ($("#cancel_check").val() != "" && ($("#cancel_bank_cheque").val() != "" || $("#cancel_bank_cheque_old").val() != "")) {
        //     docCertificateValidate += 1;
        // }
        if ($("#pan_number").val() != "" && ($("#FSSAI_certificate").val() != "" || $("#FSSAI_certificate_old").val() != "")) {
            docCertificateValidate += 1;
        }
        return docCertificateValidate;
    }

    $('.docnumber-group').blur(function() {
        //alert($("#gst_certificate").val());
    ...
    $('.certificateDocValidate').blur(function() {
        //alert($("#gst_certificate").val());
        $(".docCertificatValidat").html("");
        $(".docCertificatValidat").empty();
        $(".docCertificatValidat").remove();
        if (checkDocCertificate() < 2) {
            // $('.certificateDocValidate').after('<div class="error docCertificatValidat">Please Select Any two document from first four options are mandatory</div>');
            return false
        } else {
            $(".docCertificatValidat").html("");
            $(".docCertificatValidat").empty();
    ...
    $('#saveCustomerProfileDetails').on('submit', function(e) {
        ...
        $(".docNumberValidat").remove();
        if (checkDocNum() < 2) {
            alert("Please Select Any two document from first four options");
            // $('.docnumber-group').after('<div class="error docNumberValidat">Please Select Any two document from first four options are mandatory</div>');
            return false;
        } else {
            $(".docNumberValidat").remove();
    ...
    $('#updateCustomerProfileDetails1').on('submit', function(e) {
        e.preventDefault();
