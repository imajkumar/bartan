...
?>
			...
			<div class="MultiCarousel" data-items="1,3,5,6" data-slide="1" id="MultiCarousel" data-interval="1000">
				<div class="MultiCarousel-inner" style="transform: translateX(0px); width: 1120px;">

					<?php
					//$items = get_items($flag = '', $limit = 50, $brandId);
					...
					?>
							...
							<!-- BEGIN item -->
							<div class="item item-thumbnail itemht" style="width: 186.667px;">
								<div class="item-in">
							...
							<div class="item-info">
								...
								?>
									...
									@if($item->item_mrp)
									<div class="item-price itm-dpr">MRP:<span> ₹ {{($item->item_mrp)? $item->item_mrp:''}}</span></div>
									@endif
			...
			<?php 
            $pageNumRevers = ($pageNum > 1)? $pageNum: 2;
            $pageNum = $pageNum+1;
            $pageNumRevers = $pageNumRevers -1;
            $totalPage = ceil(count($items) /6);
            ?>
				<button class="btn btn-primary leftLst" onclick="carauselItemAppendOnClick({{$pageNo = $pageNumRevers, $perPageLimit = $limitItemCrausel}})"><</button> 
				<button class="btn btn-primary rightLst" onclick="carauselItemAppendOnClick({{$pageNo = $pageNum, $perPageLimit = $limitItemCrausel}})">>	</button>
</div>
