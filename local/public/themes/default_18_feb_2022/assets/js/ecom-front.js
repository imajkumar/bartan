    ...
    $.ajax({
        ...
        success: function(responce) {
            if (responce.status == 1) {
                ...
                // });
                $('#cartAppendHtml').empty().append(responce.CartHtml);

                $('#checkout-cart').empty().append(responce.viewCartHtml);
                //location.reload(1);
            } else {

                location.reload(responce.url);
            }
        }
    });
    //ajax
    ...
    $.ajax({
        ...
        success: function(responce) {
            if (responce['status'] == '1') {
                ...
                $('#search_keyword').html(`<b>${responce['keyword']}</b>`);
                // $('#itemsByBrand').find('.item-row').remove().end().append(responce['itemHtml']);
...
if (/Android|webOS|iPhone|iPad|Mac|Macintosh|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
    function carauselItemAppendOnClick(pageNo = 1, perPageLimit = 1) {
        ...
        $.ajax({
            type: 'GET',
            url: BASE_URL + '/getItemOnClickCarausel',
            data: formData,
            ...
            success: function(responce) {
                ...
                if (responce['success'] == true) {
                    ...
                    $('#appendItemInCarousal').empty().append(responce['ordeHtml']);
                    ...
                    $('#appendItemInCarousal').empty().append('Items not found.');
    ...
    function carauselItemAppendOnClick(pageNo = 1, perPageLimit = 6) {
...
function ShowBtnByPaymentType() {

    var payment_type = $("#payment_type").val();
    if (payment_type == "Online payment") {
        $(".showForCodOrCreaditBtnClass").hide();
        $(".showForCodOrCreaditBtnClassDefault").hide();
        $(".showForPayBtnClass").show();
        $(".showForPayBtnClassBillingDetail").show();
    } else {
        $(".showForCodOrCreaditBtnClassDefault").hide();
        $(".showForCodOrCreaditBtnClass").show();
        $(".showForPayBtnClass").hide();
        $(".showForPayBtnClassBillingDetail").hide();
    }
    //alert(perPageLimit);
...
function cachOnDeliveryOrder() {
    $("#defaultPaymentOption").validate({
        rules: {
        ...
        submitHandler: function(form) {
            ...
            $.ajax({
                ...
                type: form.method,
                data: $(form).serialize(),
                // beforeSend: function() { $(':input[type="submit"]').prop('disabled', true); },
                // complete: function() { $(':input[type="submit"]').prop('disabled', false); },
