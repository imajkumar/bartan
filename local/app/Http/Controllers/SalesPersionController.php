...
class SalesPersionController extends Controller
{
    ...
    public function myOrderListBySales(){
        ...
        $selesCustomers = DB::table('tbl_customers')
                ...
                ->where('customer_sales.se_id', $sales->user_id)
                ->orWhere('customer_sales.asm_id', $sales->user_id)
                ->orWhere('customer_sales.rsm_id', $sales->user_id)
                ->orWhere('customer_sales.nsm_id', $sales->user_id)
                ->select('tbl_customers.user_id','tbl_customers.id')
                ->get();
                $customerSalesTagArr = array();
            foreach($selesCustomers as $selesCustomer){
               
                   $customerSalesTagArr[] = $selesCustomer->user_id;
               
            }
// pr($customerSalesTagArr);
        ...
        ->orWhereIn('tbl_item_orders.customer_id', $customerSalesTagArr)
        ->get();
    ...
    public function getItemsWithCustomerBySeller(Request $request)
    {
        ...
        $brandData = DB::table('tbl_items')
            ...
            //}
             ...
             //$brands = $brandData->get();
             $brands = $brandData->paginate(9);
             $brands = $brands->withPath(url('/salesPersonItems'));
    ...
    public function salesItemSeach(Request $request)
    {
        ...
        $brandData = DB::table('tbl_items')
            ...
            ->leftJoin('tbl_item_tags', function ($tag) {
                ...
                ->select('tbl_brands.*','tbl_items.*', 'tbl_items.item_name as product_name', 'tbl_item_category.*', 'tbl_item_category.item_name as item_cat_name', 'tbl_group.*' 
                    ...
                    if(!empty($searchKey)){
                        ...
                        $brands = $brandData->Where(function($query) use ($searchKey, $brandId, $categoryId, $brandIdc){
                        ...
                        $brands = $brandData->Where(function($query) use ($searchKey, $brandId, $categoryId, $brandIdc){
                            foreach ($searchKey as $value) {
                                ...
                                // $query->orWhere('tbl_attribute_options.attribute_option_name', 'RLIKE', '[[:<:]]'.$value.'[[:>:]]');
                                $query->orWhere('tbl_item_category.item_name', 'RLIKE', '[[:<:]]'.$value.'[[:>:]]');
                                // $query->orWhere('tbl_group.g_name', 'RLIKE', '[[:<:]]'.$value.'[[:>:]]');
                    ...
                    if(!empty($keyword)){
                        $brands = $brandData->orWhere(function($query) use ($keyword, $brandId, $categoryId, $brandIdc){
                    ...
                    if(!empty($request->brandId)){
                        $brands = $brandData->Where('tbl_items.brand_id', $request->brandId);
                        $brands = $brandData->Where('tbl_brands.id', $brandIdc);
                    }
                    if(!empty($request->catId)){
                        $brands = $brandData->Where('tbl_items.cat_id', $request->catId);
                        $brands = $brandData->Where('tbl_item_category.id', $request->catId);
                    }
                    //->orderBy('tbl_items.item_id', 'desc');
            //if(count($brandId) > 0){
                $brands = $brandData->whereIn('tbl_items.brand_id', $brandId);
            ...
            $brands = $brands->withPath(url('/salesItemSearch'));
            ...
            // $requestBrandId = Request::get('brandId');
                if(!empty($keyword)){
                    ...
                    $brands = $brands->appends(['keyword' => $keyword]);
                }
                if(!empty($categoryId)){

                   
                    $brands = $brands->appends(['catId' =>$categoryId]);
                }

                if(!empty($brandIdc)){
                    $brands = $brands->appends(['brandId' => $brandIdc]);
                }
    ...
    public function salesItemSearch(Request $request)
    {
        $brandId = array();
        ...
        $requestBrandId = $request->get('brandId');
        $brandIdc = "";
        if(!empty($requestBrandId)){
            
            $brandId[] = $request->brandId;
            $brandIdc = $requestBrandId;
        }else{
            $customer = session()->get('customerForSalesPanel');
        ...
        $brandData = DB::table('tbl_items')
            ...
            ->leftJoin('tbl_item_tags', function ($tag) {
                ...
                ->select('tbl_brands.*','tbl_items.*', 'tbl_items.item_name as product_name', 'tbl_item_category.*', 'tbl_item_category.item_name as item_cat_name', 'tbl_group.*' 
                    ...
                    if(!empty($brandIdc)){
                        $brands = $brandData->Where('tbl_items.brand_id', $brandIdc);
                        $brands = $brandData->Where('tbl_brands.id', $brandIdc);
                    }
                    if(!empty($categoryId)){
                        $brands = $brandData->Where('tbl_items.cat_id', $categoryId);
                        $brands = $brandData->Where('tbl_item_category.id', $categoryId);
                    }
                    //->orderBy('tbl_items.item_id', 'desc');
            //if(count($brandId) > 0){
                $brands = $brandData->whereIn('tbl_items.brand_id', $brandId);
    ...
    public function salesItemSeach_old(Request $request)
    {
        $customer = session()->get('customerForSalesPanel');
        $brandId = array();
        if(!empty($customer)){
