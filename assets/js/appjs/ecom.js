...
$("select.productAttribute_For_add_one_by_one_attribute_item_code_23_augst_2021").change(function() {
    var selectAttrID = $(this).children("option:selected").val();
    //alert(selectAttrID);
    //ajax
    //$( '#productAttribute' ).html( "<option value=''>-SELECT-</option>" );
...
jQuery.validator.addMethod("greaterThan",
    ...
    }, 'Must be greater than Applicable from date.');
// }, 'Must be greater than {0}.');

$("#saveClassDiscount").validate({
    rules: {
    ...
    messages: {
        discount_percentage: "Please fill Valid Discount.",
        customer_type: "Please select Customer Type.",
        customer_class: "Please select Customer Class.",
        item_cat: "Please select Item Category.",
...
// if ($("#saveClassDiscount").valid() == false) {
...
function updateClassDiscountBtn() {
    if ($("#updateClassDiscount").valid() == false) {
        //alert('ffff');
        return false;
    }
...
$("#updateCategoryDiscount").validate({
    rules: {
    ...
    messages: {
        discount_percentage: "Please fill Valid Discount.",
        //customer_type: "Please select Customer Type.",
        cat_class: "Please select Customer Class.",
        item_cat: "Please select Item Category.",
...
function updateCategoryDiscountBtn() {
    ...
    if ($("#updateCategoryDiscount").valid() == false) {
        //alert('ffff');
        return false;
    }
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            $("#updateCategoryDiscount")[0].reset();
            if (response.status == 1) {
...
$("#saveCategoryDiscount").submit(function(event) {
    ...
    if ($("#saveCategoryDiscount").valid() == false) {
        //alert('ffff');
        return false;
    }
...
function cancelOrder(cancelOrder) {

    var orderIdAndStageFelter = cancelOrder.split('_');
    var itemOrderId = orderIdAndStageFelter[0];

    var custumerId = orderIdAndStageFelter[1];

    $('#cancelItemOrderId').val(itemOrderId);
    $('#custumerId').val(custumerId);

}

function cancelOrderModelByCustomer(cancelOrder) {
    ...
    $('#cancelItemOrderIdFor').val(itemOrderId);
    $('#custumerIdFor').val(custumerId);
...
$("#otherReasonForCustomer").hide();

function cancelReasonSelectionForCustomer() {

    var cancel_reason = $("#cancel_reason_For_Customer").val();

    if (cancel_reason == 'Other') {
        //alert($("#other_reason").val());
        $("#otherReasonForCustomer").show();
    } else {
        $("#otherReasonForCustomer").hide();
    }
...
function updateDeliveredOrderPaymentStatus(itemOrderIdAndStage) {
    ...
    $('#itemOrderIdForPaymentStatus').val(itemOrderId);
    // $('#shipingNumber').val(shipingNumber);
    // $('#packingNumber').val(packingNumber);
    //$("#gender").val("Male").attr("selected","selected");
    // alert(itemOrderIdAndStage);
    $('#stage').val(stage).attr("selected", "selected");
}
...
function allItemByOrderProccessModel(itemOrderId) {
    ...
    $.ajax({
        type: 'POST',
        url: BASE_URL + '/allItemByOrderProccessModel',
        data: formData,
        //dataType: "json",
        ...
        success: function(responce) {
            ...
            if (responce['success'] == true) {
                ...
                $('#allItemByOrderProccessOnModelAppend').empty().append(responce['ordeHtml']);


            } else {

                $('#allItemByOrderProccessOnModelAppend').empty().append('Recourd not found.');
...
function editItemToBePackedByItemId(itemId) {


    // $(this).prop('checked', true);

    var itemIdAndScanItemRowId = itemId.split('_');


    var itemIdByCheckBox = itemIdAndScanItemRowId[0];
    var customer_id = $("#customer_id").val();
    var order_number = $("#order_number").val();

    var scanItemRowId = itemIdAndScanItemRowId[1];
    //alert(scanItemRowId);
    ...
    $.ajax({
        ...
        success: function(responce) {
            // alert(responce.item_invt_weightmUnit);
            if (responce.status == 1) {
                // $.gritter.add({
                //     title: 'Barcode',
                //     text: responce.msg
                // });
                //alert(responce.item_invt_weight);
                ...
                $("#item_invt_weight").val(responce.item_invt_weight);
                $("#scanItemRowId").val(scanItemRowId);
...
function checkedGetBarcodeByItemId_old(itemId) {


    // if ($("#" + itemId).is(':checked')) {
    // if ($(".checkedGetBarcodeByItemId").is(':checked')) {
    //alert(44);
    //$('.checkedGetBarcodeByItemId').attr('checked', false);
    // $('.checkedGetBarcodeByItemId').prop('checked', false);
    // $(this).prop('checked', true);
    // $('.checkedGetBarcodeByItemId').prop('checked', false);
    // $(this).prop('checked', true);
...
function checkedGetBarcodeByItemId() {

    var data = $("#checkedGetBarcodeByItemIdForm").serializeArray();
    var customer_id = $("#customer_id").val();
    var order_number = $("#order_number").val();
    data.push({ name: 'customer_id', value: customer_id });
    data.push({ name: 'order_number', value: order_number });
    ...
    $.ajax({
        type: 'POST',
        url: BASE_URL + '/barcodeScanItemSave',
        //url: BASE_URL + '/checkedGetBarcodeByItemId',
        data: data,
        // data: formData,
    ...
    $.ajax({
        ...
        success: function(res) {
            ...
            $.ajax({
                ...
                data: formDataArr,
                success: function(resp) {
                    $('.shipping_country').val(resp.country_id).attr("selected", "selected");
                    $('.shipping_state').find('option').remove().end().append(resp.states_option);
                    $('.shipping_city').find('option').remove().end().append(resp.citye_option);
...
$('.shipping_country').change(function() {
    ...
    $.ajax({
        ...
        data: formData,
        success: function(res) {
            $('.shipping_state').find('option').remove().end().append(res);
        }
    });
});
...
function countPricePerPscAndRegularPrice() {

    var weight = $('#item_invt_weight').val();
    var price_per_kg = $('#price_per_kg').val();
    var set_of = $('#set_of').val();
    var price_per_pcs = 0;
    if (weight != "" && price_per_kg != "") {

        var price_per_pcs = weight * price_per_kg;
        if (!isNaN(price_per_pcs)) {

            $("#price_per_pcs").val(Math.round(price_per_pcs));
    ...
    if (price_per_pcs != "" && set_of != "") {

        var regular_price = price_per_pcs * set_of;
        if (!isNaN(regular_price)) {

            $("#regular_price").val(Math.round(regular_price));
        } else {
            $("#regular_price").val("");
        }
...
$("#ckbCheckAll").click(function() {
    $(".checkedGetBarcodeByItemId").prop('checked', $(this).prop('checked'));
});


$("#savePincode").validate({
    ...
    submitHandler: function(form) {
        ...
        $.ajax({
            ...
            type: form.method,
            //data: $(form).serialize(),
            data: new FormData($("#savePincode")[0]),
            contentType: false,
            //cache: false,
            processData: false,
            dataType: 'json',
...
$("#updatePincode").validate({
    ...
    submitHandler: function(form) {
        ...
        $.ajax({
            ...
            type: form.method,
            //data: $(form).serialize(),
            data: new FormData($("#updatePincode")[0]),
            contentType: false,
            //cache: false,
            processData: false,
            dataType: 'json',
