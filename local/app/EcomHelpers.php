    ...
    //$columns = ['title', 'subtitle', 'description'];
     //pr($searchValues);
           ...
           ->Where('tbl_items.is_visible', 1)
            ...
            ->Where(function($query) use ($searchKey, $priceFrom, $priceTo, $brandIdForCatFilter, $orderByPriceForCatFilter, $brandId){
                foreach ($searchKey as $value) {
                    //$stringKey = preg_replace('/[^A-Za-z0-9\- ]/', '', $value); remove special char
                    $query->Where('tbl_items.item_name', 'RLIKE', '[[:<:]]'.$value.'[[:>:]]');
                    //$query->orWhere('tbl_item_tags.tag_name', 'like', "%{$value}%");
...
{
    //echo $orderNumber;
    $InprocessItem = DB::table('tbl_item_orders')
    ->where('order_id', $orderNumber)
    ->where('is_Inprocess', 1)
    
    ->where('is_packed', 0)
    ->orWhere(function($query) use ($orderNumber){
        $query ->where('is_Inprocess', 2)
        ->where('order_id', $orderNumber);
        
    })->count();
    //echo $InprocessItem;exit;
...
function getNextOrderNumberTest()
{
    ...
    if ( ! $lastOrder ){
        $number = 0;
         ...
         $orederId = $lastOrder->order_id;
         //echo $orederId;
         //$actualNum = substr($lastOrder->order_id, 2);
         //echo $actualNum;
         //substr($lastOrder->order_id, 3);
 
         // $currectNum = sprintf('%d', intval($orederId) + 1);
         // echo $currectNum;
         if($orederId >= '001000'){
             //echo "GG";
             $currectNum = sprintf('%d', intval($orederId) + 1);
...
function sendSmsNewOrder($mobile, $message) 
{
    $tempid='1207164179274067613';
...
function getItemOnClickCarausel($pageNo='1', $perPageLimit='6',$brandId = array())
{
    ...
    $data = DB::table('tbl_items')
        ...
        ->orderByRaw('IF(tbl_items.trending_item = 1, null, 0)')
        ->skip($offset)
        ->take($perPageLimit);
...
function notificationCustomer(){
    $options = array(
      ...
      $data['getNewOrders'] = $getNewOrders;

      $data['pendingCustomerRegisterCount'] = $pendingCustomerRegisterCount;
      $data['getPendingCustomerRegister'] = $getPendingCustomerRegister;

      $returnNotificationHtml = view('notificationAppend', $data)->render();

        $notificationData['returnNotificationHtml'] = $returnNotificationHtml;
        //return response()->json(array('success' => true, 'ordeHtml' => $returnViewPaginateFile));

      $pusher->trigger('bartanNotificationCustomer', $event_id, $notificationData);
...
function getCustomerCart(){
    if(@Auth::user()){
        ...
        $customerCartLists = DB::table('customer_carts')->where('customer_id', Auth::user()->id)
        //->groupBy('customer_id')
        ->get();
            ...
            foreach($customerCartLists as $customerCartList){
                ...
                if(@$itemDataArr->is_visible == 1){
                    \Cart::add(array(
                        ...
                        'attributes' => array(
                            'set_of' => ($customerCartList->set_of)? $customerCartList->set_of : 0,
                        )
                    ));
                }else{
                    DB::table('customer_carts')->where('customer_id', Auth::user()->id)->where('item_id', $customerCartList->item_id)->delete();
                }
