...
class PaymentController extends Controller
{
    ...
    public function defaultPaymentOption(Request $request){
        ...
        {
            $productArr = DB::table('tbl_items')->where('item_id', $item_id)->select('invt_saleunit','is_tax_included', 'cat_id','regular_price', 'item_invt_min_order','item_cart_remarks', 'is_visible','item_name')->first();
            if($productArr->is_visible == 2){
                \Cart::remove($item_id);
                \Session::flash('error', $productArr->item_name.', This product has been removed from the website.');
                            ...
                            foreach($request->netRates[$item_id] as $netRate)
                                {
                                    ...
                                    if ($users->cutomer_state == 10) {
                                        ...
                                        $ItemOrder->price_per_kg = ($request->price_per_kg[$item_id]) ? $request->price_per_kg[$item_id] : 0;
                                        $ItemOrder->set_of_in_weight = ($request->set_of_in_weight[$item_id]) ? $request->set_of_in_weight[$item_id]:0;
                                        $ItemOrder->item_weight_in_kg = ($request->item_weight_in_kg[$item_id])? $request->item_weight_in_kg[$item_id]:0;
                                        $ItemOrder->price_per_pcs_in_weight = ($request->price_per_pcs_in_weight[$item_id])? $request->price_per_pcs_in_weight[$item_id]:0;
    ...
    public function paymentBySales(Request $request)
    {
        ...
        foreach($request->itemIdForOder as $item_id)
        {
            $productArr = DB::table('tbl_items')->where('item_id', $item_id)->select('invt_saleunit','is_tax_included', 'cat_id','regular_price', 'item_invt_min_order','item_cart_remarks', 'is_visible')->first();
            if($productArr->is_visible == 2){
                \Cart::remove($item_id);
                \Session::flash('error', $productArr->item_name.'is not active for order');
                return redirect()->back();
            }
        }
    ...
    public function codOrCreditOrderBySales(Request $request)
    {
        ...
        foreach($request->itemIdForOder as $item_id)
        {
            foreach($request->quantityForOder[$item_id] as $quantity)
            {
                ...
                @$itemDataArr = DB::table('tbl_items')->where('item_id', $item_id)->first();
           
                $CustomerCart = CustomerCart::updateOrCreate([
                    'customer_id' => $customer_id,
                    'item_id' => @$itemDataArr->item_id,
                    
                ],
                [
                    'customer_id' => $customer_id,
                    'created_by_sales_id' => $saler_id,
                    'item_id' => @$itemDataArr->item_id,
                    'qty' => @$quantity,
        ...
        if($CustomerCart){
            ...
            return response()->json(array(
                'success' =>1, 
                'msg' =>'Item added in customer cart Successfully.',
                'url' =>route('salesPersonItems')
            ));
    ...
    public function codOrCreditOrderBySales_15_feb_2022(Request $request)
    {
        $input = $request->all();
        //pr($input);
    ...
    public function codOrCreditOrderByCustomer(Request $request)
    {
        ...
        if($paymntStaus->save()){
            ...
            try {
                ...
                $sms = sendSmsNewOrder($users->phone, "Dear customer thank you for shopping with us.
                We'd like to let you know that Bartan.com has received your order ID $paymntStaus->item_order_id. and is preparing it for shipment.
                ");
                // $sms = sendSms($users->phone, "Thank you for shopping with us!
                // We'd like to let you know that Subhiksh has received your order, and is preparing it for shipment.
                // ");
        ...
        return Response::json(array(
            'status' => 1, 
            'msg' => 'Order placed.', 
            'url' => route('paymentSuccess', $order)
        ));
    }
