...
class RegisterController extends Controller
{
    ...
    */
    ...
    //    $afterRegisterCustomerDetail = User::where('mobile', $request->mobile)->orWhere('email', $request->mobile)->where('user_type',0)->first();
        if (filter_var($request->mobile, FILTER_VALIDATE_EMAIL) !== false) {
            ...
            if($customer){
                ...
                session()->forget('OtpSendTime');
                $timestamp =  $_SERVER["REQUEST_TIME"];
                $request->session()->put('OtpSendTime', $timestamp);
    ...
    public function verifyOtp(Request $request){
        ...
        //$customerInfo = User::where($field, $request->mobile)->where('otp', $request->otp)->where('user_type', 0)->first();
        $currentTime =  $_SERVER["REQUEST_TIME"];
        $OtpSendTime = session()->get('OtpSendTime');
        $totalSecoud = $currentTime - $OtpSendTime;
        // echo $totalSecoud;exit;
        if($totalSecoud > 300)  
        {
            return Response::json(array('status' => 'warning', 'msg' => 'OTP expired. Please, try again.', 'mobile' =>$request->mobile));
        }
        
        // $now = now(); // or your date as well
        // $your_date = strtotime($customerInfo->updated_at);
        ...
        // $now = Carbon::now();
        //  $created_at = Carbon::createFromFormat('Y-m-d H:i:s', $customerInfo->updated_at);
        // // echo $now;exit;
        // //$created_at = $customerInfo->updated_at;
        // // $created_at = Carbon::parse($customerInfo->updated_at);
        // //echo $customerInfo->updated_at."<br>";
        //  //echo $created_at."<br>";exit;
        // // echo $now."<br>";exit;
        //  $diffMinutes = $now->diffInMinutes($created_at);
         
        //  //$diffSecound = $created_at->diffInSeconds($now);
        // echo $diffMinutes;exit;
