    ...
    public function sendOtp(Request $request){
       ...
       //$otp = generateOtp(6);
        ...
        if (filter_var($request->mobile, FILTER_VALIDATE_EMAIL) !== false) {
            ...
            // $checkForVal = User::where('mobile', $request->mobile)->orWhere('email', $request->mobile)->where('user_type',0)->first();
            $checkForVal = User::where('email', $request->mobile)->first();
            if(!empty($checkForVal)){
 
                $this->validate($request, [
                    //'mobile' => 'email',
                    'mobile' => 'email|unique:users,email,'.$checkForVal->id,
                ]);
            }else{
                $this->validate($request, [
                    'mobile' => 'email',
            ...
            $checkForVal = User::where('mobile', $request->mobile)->first();
            //pr($checkForVal);
            if(!empty($checkForVal)){
                //  echo $checkForVal->id."";exit;
                $this->validate($request, [
                   
                    'mobile' => 'required|digits:10|unique:users,mobile,'.$checkForVal->id,
                ]);
            }else{
                $this->validate($request, [
