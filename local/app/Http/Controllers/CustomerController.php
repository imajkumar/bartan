...
class CustomerController extends Controller
{
    ...
    public function orderCancelByCustumer($orderId, $custumerId){

        DB::table('tbl_item_orders')->where('order_id', $orderId)
       ...
       //return $theme->scope('admin.order_custumer', compact('itemOrders'))->render();
   ...
   public function saveCustomerProfileDetails(Request $request)
    {
        ...
        //checking 
        $aj=0;
        if(!empty($request->business_gst_number) && $request->hasFile('gst_certificate') ){
            if(!preg_match("/^([0-9]){2}([a-zA-Z]){5}([0-9]){4}([a-zA-Z]){1}([0-9]){1}([a-zA-Z]){1}([0-9a-zA-Z]){1}?$/", $request->business_gst_number)){
                return Response::json(array('status' => 'warning', 'msg' => 'GST number is not accepted.'));
            }
            // $this->validate($request, [
            // 'business_gst_number' => 'regex:/^([0-9]){2}([a-zA-Z]){5}([0-9]){4}([a-zA-Z]){1}([0-9]){1}([a-zA-Z]){1}([0-9a-zA-Z]){1}?$/',
            // ]);
            // 'pan_number' => 'regex:/([A-Z]){5}([0-9]){4}([A-Z]){1}$/',
             
            $aj++;
        }
        if(!empty($request->pan_number) && $request->hasFile('FSSAI_certificate')){
            if(!preg_match("/([A-Z]){5}([0-9]){4}([A-Z]){1}$/", $request->pan_number)){
                return Response::json(array('status' => 'warning', 'msg' => 'Pan number is not accepted.'));
            }
            $aj++;
        }
        if(!empty($request->adhar_number) && $request->hasFile('adhar_card')){
            if(strlen(trim($request->adhar_number)) != 12 && is_numeric($request->adhar_number) != 1){
                return Response::json(array('status' => 'warning', 'msg' => 'Please fill valid Adhar number.'));
            }
            $aj++;
        }
        if(!empty($request->dl_number) && $request->hasFile('driving_license')){
            if(strlen(trim($request->dl_number)) == 0){
                return Response::json(array('status' => 'warning', 'msg' => 'Please fill cancel check number.'));
            }
            $aj++;
        }
        // if(!empty($request->cancel_check) && $request->hasFile('cancel_bank_cheque')){
        //     if(strlen(trim($request->cancel_check)) == 0){
        //         return Response::json(array('status' => 'warning', 'msg' => 'Please fill cancel check number.'));
        //     }
        //     $aj++;
        // }
        if($aj<=1){
            return Response::json(array('status' => 'warning', 'msg' => 'Please Select Any two document from first four options'));
        }
        //checking 
   ...
   public function updateCustomerProfileDetails(Request $request)
    {
        ...
        $this->validate($request, [
         ...
         //pr($request->all());
         $aj=0;
         if(!empty($request->business_gst_number1) && $request->hasFile('gst_certificate') ){
             if(!preg_match("/^([0-9]){2}([a-zA-Z]){5}([0-9]){4}([a-zA-Z]){1}([0-9]){1}([a-zA-Z]){1}([0-9a-zA-Z]){1}?$/", $request->business_gst_number)){
                 return Response::json(array('status' => 'warning', 'msg' => 'GST number is not accepted.'));
             }
             // $this->validate($request, [
             ...
             if(!empty($request->gst_certificate_old) && !empty($request->business_gst_number1)){
                    $aj++;
                }
         }
         if(!empty($request->pan_number) && $request->hasFile('FSSAI_certificate')){
            ...
            if(!empty($request->FSSAI_certificate_old) && !empty($request->pan_number)){
                   $aj++;
               }
        }
         if(!empty($request->adhar_number) && $request->hasFile('adhar_card')){
             ...
             $aj++;
                ...
                if(!empty($request->adhar_card_old) && !empty($request->adhar_number)){
                       $aj++;
                   }
            }
         if(!empty($request->dl_number) && $request->hasFile('driving_license')){
             ...
             $aj++;
                ...
                if(!empty($request->driving_license_old) && !empty($request->dl_number)){
                       $aj++;
                   }
            }
         // if(!empty($request->cancel_check) && $request->hasFile('cancel_bank_cheque')){
        ...
        $businessData = DB::table('tbl_businesses')->updateOrInsert(
            ...
            'store_name' => $request->store_name,
            'business_street_address' => $request->business_street_address,
            'business_gst_number' => $request->business_gst_number1,
            // 'business_gst_number' => $request->business_gst_number,
            'business_country' => $request->business_country,
            'business_state' => $request->business_state,
            'business_city' => $request->business_city,
    ...
    public function orderCancelByCustomerAjax(Request $request){
