...
// pr($itemOrders);
$customerdetail = get_customer_and_address_by__user_id(@$itemOrderPack->customer_id);
//pr($customerdetail);
        ...
        <div class="invoice" id="checkout-cart">
            ...
            <div class="invoice-header">
                <div class="invoice-to">
                    <address class="m-t-5 m-b-5">
                        Order No: {{$itemOrderPack->order_id}}<br>
                        Date Of Order : {{date("M d,Y", strtoTime($itemOrderPack->created_at))}}<br>
                    </address>
                </div>
                <div class="w-30">
          ...
          Email id:       {{Auth::user()->email}}<br /> -->
                        ...
                        <div class="invoice-detail">
                            ...
                            Payment status:
                            <?php
                         $paymntStaus =  DB::table('tbl_payment_status')->where('item_order_id',  $itemOrderPack->order_id)
                        ->first();
                        ?>
            ...
            <div class="invoice-content">
                ...
                <div class="table-responsive table-print">
                    <table class="table table-invoice">
                        ...
                        $item_Price = 0;
                                    ...
                                    //pr($getItemOrders);
                                    foreach($getItemOrders as $getPackedItemOrder){
                                    //foreach($itemOrders as $itemOrder){
                                             
                                        $itemOrder = DB::table('tbl_item_orders')
            
                                            //->where('tbl_item_orders.is_packed', 1)
                                            ->where('id', $getPackedItemOrder->order_item_id)
                                            //->where('packing_no', $getItemOrder->packing_no)
                            ...
                            $sumQty += optional($getPackedItemOrder)->qty;
                            //$sumQty += optional($itemOrder)->quantity;
							
							//$item_Price = $itemOrder->item_price;
							//Tax
								$productArr = DB::table('tbl_items')->where('item_id', $itemOrder->item_id)->select('invt_saleunit','is_tax_included', 'cat_id','regular_price', 'item_invt_min_order','item_cart_remarks','price_per_kg')->first();
                                //$DisacoutArrData = getDiscountAppliedByProductID(Auth::user()->id, $rowData['id'], $rowData['quantity']);
                                //ajay
                                
                                //neeraj
                                @$custClassDiscount = getCustomerClassDiscount(@$itemOrderPack->customer_id, $productArr->cat_id);
                                @$custCatDiscount = getCustomerCategoryDiscount(@$itemOrderPack->customer_id, $productArr->cat_id);
                               ...
                               //echo @$AfterDiscountPrice;exit;
                                ...
                                //$disAmt = $DisacoutArrData; //ajay
                                 //$netAmt = ($item->regular_price) - ($disAmt);

                                $netAmt = ($itemOrder->item_price) - ($disAmt);

                                //Wieght calculation
                            // if(!empty(@$getPackedItemOrder->calculation_weight) && !empty(@$item->item_invt_weight) && !empty(@$item->price_per_kg)){
                                
                            //     $pricePerKg = optional($item)->price_per_kg;
                            //     $item_set_of = optional($item)->set_of;
                            //     $ItemWeight = optional($item)->item_invt_weight;

                            if(!empty(@$getPackedItemOrder->calculation_weight) && !empty(@$itemOrder->item_weight_in_kg) && !empty(@$itemOrder->price_per_kg) && !empty(@$itemOrder->set_of_in_weight) ){
                                ...
                                $masterTtotalWeight = ($ItemWeight * $item_set_of) * $getPackedItemOrder->qty;

                                $calculatedWeight = optional($getPackedItemOrder)->calculation_weight;
                                ...
                                if($masterTtotalWeight > $calculatedWeight){
                                    ...
                                    $priceAfterWeight = $priceAfterWeight / $getPackedItemOrder->qty;
                            ...
                            //Wieght calculation
                                ...
                                // $netAmtount = $netAmt * $getPackedItemOrder->qty;
                                
                                // $netAmtount = $netAmt * $itemOrder->quantity;
								// $itemQTY = $getItemOrder->qty;
                                $netAmtount = $netAmt * $getPackedItemOrder->qty;
							$itemQTY = $getPackedItemOrder->qty;
								
								$taxArrData = getTaxAppliedByProductID(@$itemOrderPack->customer_id, $itemOrder->item_id, $getPackedItemOrder->qty);
							   ...
							   $maxCgst = 0;
									   ...
									   if (@$itemOrderPack->cutomer_state == 10) {
                            ...
                            <tr>
                                ...
                                <td class="value">{{optional($getPackedItemOrder)->qty}}
                                ...
                                <!-- <td class="value">{{optional($item)->regular_price}}</td> -->
                                ...
                                <td class="cart-summary" colspan="2">
                                    ...
                                    <div class="summary-container">
                                        ...
                                        <div class="summary-row total">
                                            ...
                                            <!-- <div class="value">{{round(@$itemOrderPack->grand_total)}}</div> -->
                                        </div>
                                    </div>
                                </td>
                            </tr>
                ...
                <div class="invoice-price-right">
                    <small>TOTAL</small> <span class="f-w-600">₹{{@$itemOrderPack->grand_total}}</span>
                </div>
            </div> -->
