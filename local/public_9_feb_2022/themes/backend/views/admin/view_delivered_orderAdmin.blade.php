            ...
            <div class="invoice-content">
                ...
                <div class="table-responsive table-print">
                    <table class="table table-invoice">
                        ...
                        $item_Price = 0;
                                    //pr($itemOrders);
                                    foreach($itemDeliverdOrders as $itemDeliverdOrder){
                                    // foreach($itemOrders as $itemOrder){

                                        $itemOrder = DB::table('tbl_item_orders')
            
                                            ->where('packing_no', $itemDeliverdOrder->packing_no)
                                            ->where('customer_id', $itemDeliverdOrder->customer_id)
                                            ->where('item_id', $itemDeliverdOrder->item_id)
                                            ->first();
							...
							//Tax
                               ...
                               //echo @$AfterDiscountPrice;exit;
                                ...
                                if(!empty(@$itemDeliverdOrder->calculation_weight) && !empty(@$itemOrder->item_weight_in_kg) && !empty(@$itemOrder->price_per_kg) && !empty(@$itemOrder->set_of_in_weight) ){
                                   ...
                                   $masterTtotalWeight = ($ItemWeight * $item_set_of) * $itemDeliverdOrder->qty;
  
                                   $calculatedWeight = optional($itemDeliverdOrder)->calculation_weight;
                                   ...
                                   if($masterTtotalWeight > $calculatedWeight){
                                       ...
                                       $priceAfterWeight = $priceAfterWeight / $itemDeliverdOrder->qty;
                                ...
                                $netAmtount = $netAmt * $itemDeliverdOrder->qty;
								$itemQTY = $itemDeliverdOrder->qty;
								
								$taxArrData = getTaxAppliedByProductID(@$itemOrders[0]->customer_id, $itemOrder->item_id, $itemDeliverdOrder->qty);
                            ...
                            <tr>
                                ...
                                <td class="value">{{optional($itemDeliverdOrder)->qty}}
