...
$('#is_billing_address_same').change(function() {
    ...
    if ($("#is_billing_address_same").is(':checked')) {
        ...
        $('#biling_address').val(communicationAddress);
        $('#biling_postalcode').val(business_postal_code);
        $('#biling_country').val(business_country);
        $('#biling_state').val(business_state);
        var formData = {
        ...
        $('#biling_city').val(business_city).attr("selected", "selected");

    } else {

        $('#biling_address').val('').prop("placeholder", "Please enter address");
        $('#biling_postalcode').val('').prop("placeholder", "Please enter postal code");
        $('#biling_country').val('').prop("selectedIndex", 0);

        $('#biling_state').val('').append(`<option value="">
        Please select state</option>`);



        $('#biling_city').find('option').remove().end().append(`<option value="">
        Please select state</option>`);
    }
