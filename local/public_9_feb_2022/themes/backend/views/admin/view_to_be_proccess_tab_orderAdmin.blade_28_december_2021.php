                    ...
                    <tbody>
                        ...
                        $item_Price = 0;
                                    ...
                                    foreach($itemOrders as $itemOrder){
                                        ...
                                        //$itemOrder = $itemOrder;
                                        // pr($itemOrder);
                                        $ScanitemOrder = DB::table('to_be_packed_scan_items')
            
                                        ->where('is_packed', 0)
                                        ->where('packing_no', null)
                                        ->where('customer_id', $itemOrder->customer_id)
                                        ->where('order_number', $itemOrder->order_id)
                                        ->where('item_id', $itemOrder->item_id)
                                        ->where('order_item_id', $itemOrder->id)
							...
							//Tax
                               ...
                               //echo @$AfterDiscountPrice;exit;
                                ...
                                if($itemOrder->item_weight_in_kg != 0 && $itemOrder->set_of_in_weight != 0 && $itemOrder->price_per_kg != 0){
                                    ...
                                    $masterTtotalWeight = ($ItemWeight * $item_set_of) * $ScanitemOrder->quantity;
    
                                    $calculatedWeight = optional($ScanitemOrder)->calculation_weight;
                                    ...
                                    if($masterTtotalWeight > $calculatedWeight){
                                        ...
                                        $priceAfterWeight = $priceAfterWeight / $ScanitemOrder->quantity;
                                ...
                                $netAmtount = $netAmt * $ScanitemOrder->quantity;
								$itemQTY = $ScanitemOrder->quantity;
								
								$taxArrData = getTaxAppliedByProductID(@$itemOrders[0]->customer_id, $itemOrder->item_id, $ScanitemOrder->quantity);
                        ...
                        <tr>
                            ...
                            <td class="value">{{optional($ScanitemOrder)->quantity}}

                            <br/><br/>{{(optional($itemOrder)->set_of > 0)? '('.optional($itemOrder)->set_of.' Pcs)':''}}
                            </td>
                            <td class="value">{{optional($itemOrder)->unit}}</td>

                            <td class="value">{{$masterTtotalWeight}} - {{(@$calculatedWeight) ? @$calculatedWeight : $masterTtotalWeight}}</td> 
                    ...
                    <tr>
                        ...
                        <td class="cart-summary" colspan="10">
                                    ...
                                    <?php 
                                            ...
                                            ?>
                                    <p style="font-size: 1.1rem;"><strong> <?php @$InWord = numberTowords(round(@$grandTotal));?>{{@$InWord}} only</strong></p>
